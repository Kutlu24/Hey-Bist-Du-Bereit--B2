...
async function loadModul1() {
  try {
    ...
    // Tarih filtresi: karte_datum <= bugün UTC
    const _now = new Date();
    const _todayUTC = Date.UTC(_now.getUTCFullYear(), _now.getUTCMonth(), _now.getUTCDate());
    m1Vocab = m1All.filter(r => {
      const kts = r["karte_datum"];
      return typeof kts === "number" && kts <= _todayUTC;
    });
    // _audioReady: tüm kelimeler için true (ses filtresi kaldırıldı)
    m1All.forEach(r => { r._audioReady = true; });
