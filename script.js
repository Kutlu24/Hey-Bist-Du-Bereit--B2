...
async function loadModul1() {
  try {
    ...
    // Bugünün sonu: 23:59:59.999 UTC
    const TODAY_UTC_END = Date.UTC(_y, _m, _d + 1); // yarının 00:00:00 = bugünün tam sonu

    const K_AUDIO_TS = "audio_datum";
    const K_KARTE_TS = "karte_datum";

    // JSON tarihleri -1ms ile kaydedilmiş (ör: 19.03 → 19.03 23:59:59.999)
    // "Bugün veya geçmişte" = ts <= TODAY_UTC_END
    m1All.forEach(r => {
      const ats = r[K_AUDIO_TS];
      const kts = r[K_KARTE_TS];
      r._audioReady = (typeof ats === "number") && (ats < TODAY_UTC_END);
      r._karteReady = (typeof kts === "number") && (kts < TODAY_UTC_END);
    });
